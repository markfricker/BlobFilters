...
\IfFileExists{figures/demoSeg_log_WS.pdf}{%
\begin{figure*}
  \includegraphics[width=\linewidth]{figures/demoSeg_log_WS.pdf}
  \caption{\texttt{logEnhance} parameter sweep.
    Columns: raw image~$|$~best-\mbox{F1} watershed result for each of the
    four $\sigma$ configurations ($[2\!-\!4]$, $[3\!-\!5]$, $[4\!-\!6]$,
    $[3\!-\!6]$)~$|$~Cellpose GT\@.
    F1 score and object count are annotated in each panel.}
  \label{fig:demoSeg-log}
\end{figure*}
}{%
\begin{quote}\textit{Run \texttt{demoSegmentation.m} with
  \texttt{doExport = true} to generate
  \texttt{demoSeg\_log\_WS.pdf}.}\end{quote}
}

\IfFileExists{figures/demoSeg_fib_WS.pdf}{%
\begin{figure*}
  \includegraphics[width=\linewidth]{figures/demoSeg_fib_WS.pdf}
  \caption{\texttt{fiberEnhance} parameter sweep.
...
\begin{quote}\textit{Run \texttt{demoSegmentation.m} with
  \texttt{doExport = true} to generate
  \texttt{demoSeg\_fib\_WS.pdf}.}\end{quote}
}

\IfFileExists{figures/demoSeg_capS_WS.pdf}{%
\begin{figure*}
  \includegraphics[width=\linewidth]{figures/demoSeg_capS_WS.pdf}
  \caption{\texttt{capsuleEnhance} (single mode) parameter sweep.
...
\begin{quote}\textit{Run \texttt{demoSegmentation.m} with
  \texttt{doExport = true} to generate
  \texttt{demoSeg\_capS\_WS.pdf}.}\end{quote}
}

\IfFileExists{figures/demoSeg_capD_WS.pdf}{%
\begin{figure*}
...
\begin{quote}\textit{Run \texttt{demoSegmentation.m} with
  \texttt{doExport = true} to generate
  \texttt{demoSeg\_capD\_WS.pdf}.}\end{quote}
}

\IfFileExists{figures/demoSeg_rod_WS.pdf}{%
\begin{figure*}
  \includegraphics[width=\linewidth]{figures/demoSeg_rod_WS.pdf}
  \caption{\texttt{rodGranulometryEnhance} parameter sweep.
...
\begin{quote}\textit{Run \texttt{demoSegmentation.m} with
  \texttt{doExport = true} to generate
  \texttt{demoSeg\_rod\_WS.pdf}.}\end{quote}
}
