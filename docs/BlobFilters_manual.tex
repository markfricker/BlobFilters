...
%%
%% Compile sequence (pdflatex only -- DO NOT run bibtex)
%%   pdflatex BlobFilters_manual
%%   pdflatex BlobFilters_manual
%%
%% If you see "duplicate identifier" warnings or a natbib author-year error:
%%   delete BlobFilters_manual.bbl and BlobFilters_manual.aux, then recompile twice.
%%   These are caused by a stale .bbl file from a previous bibtex run.
%%
%% NOTE: tufte-book loads natbib unconditionally. \PassOptionsToPackage below
%% forces numeric citations so that bare \bibitem{key} entries are valid.
%% Without this, natbib defaults to author-year mode and errors on our \bibitem.
%
% Force natbib (loaded by tufte-book) to use numeric citations.
% Must appear before \documentclass so the option is set before natbib loads.
...
\usepackage{xcolor}
\usepackage{listings}
% hyperref must be loaded last to avoid overwriting Tufte's margin-citation machinery
\usepackage{hyperref}
...
structures--- particularly rod-shaped organelles ---against a noisy fluorescence
...
\subsection*{Mitochondria segmentation results}

\begin{figure*}
  \includegraphics[width=\linewidth]{figures/demoSeg_log_WS.pdf}
  \caption{\texttt{logEnhance} parameter sweep on \texttt{mitImagecrop.mat}.
    Columns: raw image~$|$~best-\mbox{F1} watershed result for each of four
    $\sigma$ configurations ($[2\!-\!4]$, $[3\!-\!5]$, $[4\!-\!6]$,
    $[3\!-\!6]$)~$|$~Cellpose GT (Figure~\ref{fig:gt-cyto3}).
    Panel titles show the achieved F1 score and object count.}
  \label{fig:demoSeg-log}
\end{figure*}
...
\newthought{The classical rod detectors} in this chapter are evaluated against
\texttt{cellposeEnhance} (Section~\ref{sec:cellpose}) as the deep-learning
reference baseline.  The ground truth used throughout is \texttt{cyto3} at
$\mathtt{cellProb}=0$, $\mathtt{flowThreshold}=0.8$,
$\mathtt{minSize}=64$\,px$^2$ (see Section~\ref{sec:cellpose-sweep} for the
parameter sweep that selected these settings).

\begin{figure*}
  \includegraphics[width=\linewidth]{figures/cellpose_sweep_labels_cyto3.pdf}
  \caption{Cellpose \texttt{cyto3} ground-truth segmentation on the cropped
    mitochondria image (\texttt{mitImagecrop.mat}).
    Column~1: raw image.  Columns 2--4: instance label maps
    (\texttt{label2rgb}) for $\mathtt{flowThreshold} \in \{0.60, 0.80, 1.00\}$
    at the recommended $\mathtt{cellProb}=0$.  The panel at
    $\mathtt{flowThreshold}=0.80$ is used as ground truth for all
    classical-enhancer sweeps below.  Black pixels are unlabelled background.}
  \label{fig:gt-cyto3}
\end{figure*}
...
\begin{figure*}
  \includegraphics[width=\linewidth]{figures/demoSeg_fib_WS.pdf}
  \caption{\texttt{fiberEnhance} parameter sweep on \texttt{mitImagecrop.mat}.
    Three cross-sectional width configurations (\texttt{stack} mode):
    $\mathtt{widths}=[8,9,10]$, $[9,10,11,12]$, $[10,12,14]$,
    benchmarked against the Cellpose GT (Figure~\ref{fig:gt-cyto3}).}
  \label{fig:demoSeg-fib}
\end{figure*}
...
\begin{figure*}
  \includegraphics[width=\linewidth]{figures/demoSeg_capS_WS.pdf}
  \caption{\texttt{capsuleEnhance} (single mode) parameter sweep on
    \texttt{mitImagecrop.mat}.
    Three half-length / width configurations (12 orientations each):
    $(\ell=[8,12,16], w=8)$, $([12,16,20], w=10)$, $([16,20,24], w=10)$,
    benchmarked against the Cellpose GT (Figure~\ref{fig:gt-cyto3}).}
  \label{fig:demoSeg-capS}
\end{figure*}

\begin{figure*}
  \includegraphics[width=\linewidth]{figures/demoSeg_capD_WS.pdf}
  \caption{\texttt{capsuleEnhance} (DoC mode) 2-D parameter sweep.
    Block layout: 3 rows ($\mathtt{wideWidth} \in \{16, 20, 24\}$\,px)
    $\times$ 6 columns (raw~$|$~$\alpha \in \{0.4, 0.5, 0.6, 0.7\}$~$|$~GT).
    Inner width fixed at 10\,px; half-lengths $[12,16,20,24]$;
    12 orientations.}
  \label{fig:demoSeg-capD}
\end{figure*}
...
\begin{figure*}
  \includegraphics[width=\linewidth]{figures/demoSeg_rod_WS.pdf}
  \caption{\texttt{rodGranulometryEnhance} parameter sweep on
    \texttt{mitImagecrop.mat}.
    Three full line-length configurations (12 orientations each):
    $[16,20,28]$, $[20,28,36,48]$, $[16,20,28,36,48]$\,px,
    benchmarked against the Cellpose GT (Figure~\ref{fig:gt-cyto3}).}
  \label{fig:demoSeg-rod}
\end{figure*}
...
\noindent\textbf{Results} (n\,objects / coverage\,\%, \texttt{minSize}~=~64\,px$^2$).

% --- cyto3, default nIter table ------------------------------------------
\IfFileExists{figures/cellpose_sweep_table_cyto3.tex}{%
\begin{table}
  \caption{\texttt{cyto3}, default \texttt{nIter}.}
  \centering\small
  \input{figures/cellpose_sweep_table_cyto3}
\end{table}
}{%
\begin{quote}\textit{Run \texttt{sweepCellpose.m} with \texttt{doExport = true}
  to generate \texttt{cellpose\_sweep\_table\_cyto3.tex}.}\end{quote}
}

% --- cyto3, nIter=2000 table ---------------------------------------------
\IfFileExists{figures/cellpose_sweep_table_cyto3_nIter-2000.tex}{%
\begin{table}
  \caption{\texttt{cyto3}, \texttt{nIter}~=~2000.}
  \centering\small
  \input{figures/cellpose_sweep_table_cyto3_nIter-2000}
\end{table}
}{%
\begin{quote}\textit{Run \texttt{sweepCellpose.m} with \texttt{doExport = true}
  to generate \texttt{cellpose\_sweep\_table\_cyto3\_nIter-2000.tex}.}\end{quote}
}

% --- bact\_fluor\_cp3 table -----------------------------------------------
\IfFileExists{figures/cellpose_sweep_table_bact_fluor_cp3.tex}{%
\begin{table}
  \caption{\texttt{bact\_fluor\_cp3}, default \texttt{nIter}.}
  \centering\small
  \input{figures/cellpose_sweep_table_bact_fluor_cp3}
\end{table}
}{%
\begin{quote}\textit{Run \texttt{sweepCellpose.m} with \texttt{doExport = true}
  to generate \texttt{cellpose\_sweep\_table\_bact\_fluor\_cp3.tex}.}\end{quote}
}
...
% -------------------------------------------------------------------------
\section{\texttt{demoSegmentation}}
\label{sec:demoSeg}

\newthought{This script} benchmarks all five classical rod detectors against a
Cellpose ground truth on the cropped mitochondria image
(\texttt{mitImagecrop.mat}).  For each enhancer, a grid of parameter
configurations is swept together with a per-task watershed grid; the
best-\mbox{F1} watershed result for every configuration is shown in a single
figure.  A final HSV overlay figure compares the best result per enhancer
on a perceptually uniform background.

\medskip
\noindent\textbf{Ground truth.}\quad
\texttt{cellposeEnhance} with \texttt{cyto3}, $\mathtt{diameter}=10$\,px,
$\mathtt{cellProb}=0$, $\mathtt{flowThreshold}=0.8$,
$\mathtt{minSize}=64$\,px$^2$.

\medskip
\noindent\textbf{Pipeline.}
\begin{enumerate}
  \item OGS pre-filtering ($\sigma_\parallel=4$, $\sigma_\perp=1.5$,
        8 orientations).
  \item For each enhancer: sweep parameter configurations $\times$ watershed
        combos; retain best-\mbox{F1} result per configuration.
  \item One figure per enhancer (Figures~\ref{fig:demoSeg-log}--\ref{fig:demoSeg-rod});
        HSV overlay summary (Figure~\ref{fig:demoSeg-hsv}).
\end{enumerate}

\noindent\textbf{Watershed parameters swept.}

\marginnote{%
  Capsule thresholds are raised to $[0.30$--$0.50]$ and hysteresis lower
  bounds to $\{0.20, 0.25\}$ because capsule maps carry a higher background
  pedestal than LoG or granulometry maps.
}

\begin{itemize}
  \item \texttt{threshold}: $[0.20, 0.25, 0.30, 0.35, 0.40]$ (log, fiber,
        rod); $[0.30, 0.35, 0.40, 0.45, 0.50]$ (capsule single and DoC).
  \item \texttt{hMinima}: $[0.02, 0.05, 0.10]$.
  \item \texttt{minArea}: $[20, 50]$\,px$^2$.
  \item Hysteresis \texttt{threshLow}: none / $0.10$ / $0.15$ (default);
        none / $0.20$ / $0.25$ (capsule).
\end{itemize}

The per-enhancer sweep figures are presented in context alongside each
algorithm's description:
Figure~\ref{fig:demoSeg-log} (\texttt{logEnhance},
Section~\ref{sec:log}),
Figure~\ref{fig:demoSeg-fib} (\texttt{fiberEnhance},
Section~\ref{sec:fiber}),
Figures~\ref{fig:demoSeg-capS} and~\ref{fig:demoSeg-capD}
(\texttt{capsuleEnhance} single and DoC modes,
Section~\ref{sec:capsule}),
and Figure~\ref{fig:demoSeg-rod} (\texttt{rodGranulometryEnhance},
Section~\ref{sec:rodgran}).
The HSV overlay below (Figure~\ref{fig:demoSeg-hsv}) brings all methods
together for a direct comparison.

\IfFileExists{figures/demoSeg_HSV_overlay.pdf}{%
\begin{figure*}
  \includegraphics[width=\linewidth]{figures/demoSeg_HSV_overlay.pdf}
  \caption{HSV overlay summary: best watershed result per enhancer compared
    with the Cellpose ground truth.
    Hue encodes instance identity (golden-ratio hue sequence, fixed seed for
    reproducibility), saturation is 1 for labelled pixels and 0 for background
    (rendered as greyscale), and value is the normalised original image
    intensity.  This overlay combines instance colour with image texture,
    making it straightforward to assess boundary placement alongside
    segmentation accuracy.}
  \label{fig:demoSeg-hsv}
\end{figure*}
}{%
\begin{quote}\textit{Run \texttt{demoSegmentation.m} with
  \texttt{doExport = true} to generate
  \texttt{demoSeg\_HSV\_overlay.pdf}.}\end{quote}
}

% -------------------------------------------------------------------------
\section{\texttt{sweepSegmentation}}
\label{sec:sweepSeg}

\newthought{sweepSegmentation} extends \texttt{demoSegmentation} with two
additional analyses: a hysteresis vs.\ standard-threshold comparison and a
boundary-refinement stage that applies \texttt{refineSegment} to each best
watershed result.

Two refinement methods are compared:
\begin{description}
  \item[\texttt{chanvese}] Chan--Vese active contours~\cite{Chan2001}
    (\texttt{activecontour}, 100 iterations) expand or contract each
    watershed object boundary to minimise a piecewise-constant energy
    functional.  Useful when watershed boundaries are displaced by
    intensity gradients.
  \item[\texttt{dilate}] Voronoi-style dilation: each object is expanded
    into its Voronoi territory up to a foreground intensity threshold,
    then small fragments are removed.  Faster than Chan--Vese and often
    sufficient for well-separated objects.
\end{description}

\noindent\textbf{Output.}\quad Console tables and three MATLAB figures
(not exported to PDF):
\begin{description}
  \item[Figure~1] Best watershed result per enhancer vs.\ Cellpose GT\@.
  \item[Figure~2] Refinement comparison for the top-ranked enhancer:
    unrefined WS~$|$~$+$chanvese~$|$~$+$dilate~$|$~GT, with $\Delta$F1
    annotations.
  \item[Figure~3] Best hysteresis-threshold result per enhancer.
\end{description}

The console output includes three ranked tables: the full enhancer $\times$
watershed sweep (F1 / Precision / Recall / IoU), a hysteresis vs.\
standard-threshold $\Delta$F1 comparison, and a WS $\to$ chanvese / dilate
refinement delta table.
...
\textit{IEEE Trans.\ Image Process.} \textbf{10}(2):266--277.
