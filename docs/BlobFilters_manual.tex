...
\begin{figure*}
  ...
  \caption{\texttt{cyto3} parameter sweep heatmaps.  Left: number of detected
    objects.  Right: foreground coverage~(\%).  Rows correspond to
    \texttt{cellProb} $\in \{-1, 0, +1\}$; columns to
    \texttt{flowThreshold} $\in \{0.60, 0.80, 1.00\}$.
    The cell at $\mathtt{cellProb}=0$, $\mathtt{flowThreshold}=0.8$ gives
    the best balance of object count and boundary tightness.}
  \label{fig:cellpose-sweep-heatmaps}
\end{figure*}
}{%
\begin{quote}
  \textit{Figure not available: run \texttt{sweepCellpose.m} (with
  \texttt{doExport = true}) to generate
  \texttt{cellpose\_sweep\_heatmaps\_cyto3.pdf}.}
\end{quote}
}
...
\begin{figure*}
  ...
  \caption{Visual label grid for the \texttt{cyto3} sweep.  Each row is a
    \texttt{cellProb} value ($-1$, $0$, $+1$); the first column shows the raw
    image annotated with the row label, and columns 2--4 show instance labels
    (\texttt{label2rgb}) for \texttt{flowThreshold} $= 0.60$, $0.80$,
    $1.00$ respectively.  Black pixels are background.}
  \label{fig:cellpose-sweep-labels}
\end{figure*}
}{%
\begin{quote}
  \textit{Figure not available: run \texttt{sweepCellpose.m} (with
  \texttt{doExport = true}) to generate
  \texttt{cellpose\_sweep\_labels\_cyto3.pdf}.}
\end{quote}
}
